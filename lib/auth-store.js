...
const LEGACY_STATE_KEY = "khulan:auth:v4:state";
...
const ONE_YEAR_EXTENSION_ID = "existing-users-2026-09-13";
const ONE_YEAR_EXTENSION_KEY = `khulan:admin-operation:${ONE_YEAR_EXTENSION_ID}`;
...
export async function verifyPassword(password, encoded = "") {
  try {
    if (!encoded.includes("$") && encoded.includes(":")) {
      ...
      const actual = await pbkdf2Async(String(password), salt, 210000, 32, "sha256");
...
function normalizeLegacyUser(record) {
  const username = normalizeUsername(record?.username);
  if (!record?.id || !username || !record.password) return null;
  return {
    id: String(record.id),
    role: "user",
    username,
    name: String(record.name || username).normalize("NFKC").trim(),
    phone: normalizePhone(record.phone),
    passwordHash: String(record.password),
    authVersion: Number(record.sessionVersion || 0),
    status: String(record.status || "pending_payment"),
    paymentAttempts: record.paymentSubmittedAt ? 1 : 0,
    accessUntil: legacyDate(record.accessUntil),
    createdAt: legacyDate(record.createdAt) || new Date(0).toISOString(),
    updatedAt: legacyDate(record.createdAt) || new Date(0).toISOString(),
    ...(record.paymentSubmittedAt ? { paymentSubmittedAt: legacyDate(record.paymentSubmittedAt) } : {}),
  };
}

async function legacyUsers() {
  const state = parseRecord(await getStore().get(LEGACY_STATE_KEY));
  if (!Array.isArray(state?.users)) return [];
  return state.users.map(normalizeLegacyUser).filter(Boolean);
}

async function findLegacyUserByUsername(username) {
  const normalized = normalizeUsername(username);
  return (await legacyUsers()).find((user) => user.username === normalized) || null;
}
...
export async function createUser(input) {
  ...
  if (await findLegacyUserByUsername(clean.username)) {
    throw new AppError("Энэ нэвтрэх нэр бүртгэлтэй байна.", 409, "USERNAME_EXISTS");
  }
  const db = getStore();
...
export async function getUserById(id) {
  if (!id) return null;
  const user = parseRecord(await getStore().get(userKey(id)));
  if (user) return user;
  return (await legacyUsers()).find((legacyUser) => legacyUser.id === String(id)) || null;
}

export async function findUserByUsername(username) {
  const id = await getStore().get(usernameKey(username));
  return id ? getUserById(id) : findLegacyUserByUsername(username);
}
...
export async function saveUser(user) {
  ...
  const mappedId = await db.get(key);
  if (mappedId && String(mappedId) !== String(user.id)) {
    throw new AppError("Энэ нэвтрэх нэр өөр бүртгэлтэй давхардсан байна.", 409, "USERNAME_EXISTS");
  }
  if (!mappedId) await db.set(key, user.id, { nx: true });
  await db.set(userKey(user.id), JSON.stringify(user));
  await db.sadd(USER_SET, user.id);
  return user;
}
...
export async function listUsers() {
  const db = getStore();
  const ids = await db.smembers(USER_SET);
  const byUsername = new Map();
  for (const id of ids) {
    const user = parseRecord(await db.get(userKey(id)));
    if (user) byUsername.set(normalizeUsername(user.username), publicUser(user));
  }
  for (const user of await legacyUsers()) {
    if (!byUsername.has(user.username)) byUsername.set(user.username, publicUser(user));
  }
...
function addUtcYear(timestamp) {
  const date = new Date(timestamp);
  date.setUTCFullYear(date.getUTCFullYear() + 1);
  return date.toISOString();
}

export async function extendCurrentEligibleUsersOneYear(adminUsername) {
  const db = getStore();
  const previous = parseRecord(await db.get(ONE_YEAR_EXTENSION_KEY));
  if (previous) return { ...previous, alreadyApplied: true };

  const users = await listUsers();
  const adminName = normalizeUsername(process.env.KHULAN_ADMIN_USERNAME || "admin");
  ...
  for (const listed of users) {
    if (normalizeUsername(listed.username) === adminName) continue;
    if (listed.status !== "active" && listed.status !== "expired") continue;
    const user = await getUserById(listed.id);
    if (!user) continue;
    const extensionIds = Array.isArray(user.accessExtensionIds) ? user.accessExtensionIds : [];
    if (extensionIds.includes(ONE_YEAR_EXTENSION_ID)) continue;

    const wasActive = effectiveStatus(user) === "active";
    const base = wasActive && Number.isFinite(Date.parse(user.accessUntil))
      ? user.accessUntil
      : appliedAt;
    user.status = "active";
    user.accessUntil = addUtcYear(base);
    user.accessExtensionIds = [...extensionIds, ONE_YEAR_EXTENSION_ID];
    user.accessExtendedAt = appliedAt;
    user.accessExtendedBy = adminUsername;
    await saveUser(user);
    extendedCount += 1;
  ...
  const result = { appliedAt, extendedCount, activeCount, expiredCount };
  await db.set(ONE_YEAR_EXTENSION_KEY, JSON.stringify(result));
